...
process_data = {}
for row in data:
    proc_count = int(row['Processes'])
    if proc_count not in process_data:
        process_data[proc_count] = []
    process_data[proc_count].append(row)
...
summary = []
for proc_count, rows in process_data.items():
...
summary.sort(key=lambda x: x['proc_count'])

# Calculate speedup and efficiency
if len(summary) > 0:
    base_time = next((item['avg_time'] for item in summary if item['proc_count'] == 1), None)
    ...
    for item in summary:
        ...
        item['efficiency'] = item['speedup'] / item['proc_count'] * 100 if item['proc_count'] > 0 else 0
...
if len(summary) > 0:
    thread_counts = [item['proc_count'] for item in summary]
    ...
    print("\nPerformance Summary:")
    print("-" * 80)
    print(f"{'Processes':<10} {'Avg Time(s)':<12} {'Speedup':<10} {'Efficiency(%)':<14} {'Memory(MB)':<10}")
    print("-" * 80)
    for item in summary:
        print(f"{item['proc_count']:<10} {item['avg_time']:<12.2f} {item['speedup']:<10.2f} {item['efficiency']:<14.2f} {item['avg_memory']/1024:<10.2f}")
else:
